_vehiclesLightUnarmed append ["I_G_Offroad_01_armor_base_lxWS"];
_vehiclesLightArmed append ["I_G_Offroad_01_armor_armed_lxWS"];
_vehiclesAt append ["I_G_Offroad_01_armor_AT_lxWS"];
_vehicleAA append ["I_Tura_Truck_02_aa_lxWS"];

_vehiclesMedical append ["a3a_ION_Truck_02_medical_F"];

_vehiclesCivCar append ["C_Offroad_lxWS","C_Truck_02_racing_lxWS"];
_CivTruck append ["C_Truck_02_cargo_lxWS", "C_Truck_02_flatbed_lxWS"];
_staticAA append ["I_Tura_ZU23_lxWS"];

///////////////////////////
//  Rebel Starting Gear  //
///////////////////////////

_initialRebelEquipment append [
    "Camera_lxWS"
];

_rebUniforms append [
    "U_lxWS_ION_Casual1",
    "U_lxWS_ION_Casual2",
    "U_lxWS_ION_Casual3",
    "U_lxWS_ION_Casual4",
    "U_lxWS_ION_Casual5",
    "U_lxWS_SFIA_deserter",
    "U_lxWS_Djella_02_Brown",
    "U_lxWS_Djella_02_Grey",
    "U_lxWS_Djella_02_Green",
    "U_lxWS_Djella_02_Sand"
];

_headgear append [
    "lxWS_H_Bandanna_blk_hs",
    "H_Cap_headphones_ion_lxws",
    "lxWS_H_CapB_rvs_blk_ION",
    "lxWS_H_Headset",
    "H_Beret_Headset_lxWS"
];

/////////////////////
///  Identities   ///
/////////////////////

_faces append [
"lxWS_African_Head_01",
"lxWS_African_Head_03",
"lxWS_African_Head_04",
"lxWS_African_Head_02",
"lxWS_Said_Head",
"lxWS_African_Head_Old_Bard",
"lxWS_African_Head_05",
"CamoHead_Persian_01_F", 
"CamoHead_Persian_02_F", 
"CamoHead_Persian_03_F",  
"lxWS_African_Head_Old",
"lxWS_Gustavo_Head",
"lxWS_Journalist_Head",
"lxWS_Givens_Head"
];

_goggles append ["G_Balaclava_blk_lxWS", "G_Balaclava_oli_lxWS", "G_Balaclava_snd_lxWS", "G_Combat_lxWS", "G_Headset_lxWS"];
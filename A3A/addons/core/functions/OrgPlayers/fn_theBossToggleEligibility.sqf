private _titleStr = localize "STR_A3A_fn_orgp_tvtogeli_titel";
...
if (_playerX getVariable ["eligible",false]) then
{
	...
	if (_playerX == theBoss) then
	{
		if(!isNull _newBoss && isPlayer _newBoss) then
		{
			if ([_newBoss] call A3A_fnc_makePlayerBossIfEligible) then {
				_text = format [localize "STR_A3A_fn_orgp_tvtogeli_resign_choosing", name _newBoss];
			}
			else {
				_text = format [localize "STR_A3A_fn_orgp_tvtogeli_resign_chosen", name _newBoss];
			};
		}
		else {
	...
	{
		_text = localize "STR_A3A_fn_orgp_tvtogeli_eligible_no";
	};
}
else
{
	...
	_text = localize "STR_A3A_fn_orgp_tvtogeli_eligible_yes";
};

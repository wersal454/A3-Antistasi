...
switch (_type) do {
	...
	case "CON": {
		...
		_possibleFrontlineMarkers = [outposts + resourcesX select {[_x] call A3A_fnc_isFrontline}] call _findIfNearAndHostile;
		if (count _possibleMarkers == 0) then {
			if (!_silent) then {
			...
			private _Fronlinesite = selectRandom _possibleFrontlineMarkers ///if (_possibleFrontlineMarkers isNotEqualTo []) then {} else {selectRandom _possibleMarkers};
